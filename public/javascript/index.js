...
window.addEventListener("load", () => {
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      if (event.target.occupation.value != "")
        character.occupation = event.target.occupation.value;
      if (event.target.weapon.value != "")
        character.weapon = event.target.weapon.value;
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      const exito = await charactersAPI.createOneRegister({
        name,
        occupation,
        weapon,
        cartoon,
...
function showSuccessButton(exito, tipo) {
  ...
  //console.log(exito);
  if (exito) {
    button.classList.add("button-sucess");
    setTimeout(() => {
      button.classList.remove("button-sucess");
    }, 4000);
  } else {
